if (typeof Plotly === 'undefined') {
    ...
    const showIndividualPoints = {{SHOW_INDIVIDUAL_POINTS}};

    window.AverageBarChart = {
        plotDiv: null,
        ...
        initialize: function() {
            ...
            if (!plotData || !plotData.data || plotData.data.length === 0) {
                if (window.AndroidBridge) {
                    window.AndroidBridge.onPlotError('No data provided for average bar chart');
                }
                return;
            }
        ...
        toggleIndividualPoints: function(show) {
            ...
            Plotly.react(this.plotDiv, updatedData, this.currentLayout, this.currentConfig)
                .then(function() {
                    if (window.AndroidBridge) {
                        window.AndroidBridge.onPlotUpdated('Individual points toggled');
                    }
                });
        },
