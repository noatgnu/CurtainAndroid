    ...
    window.ViolinPlot = {
        ...
        initialize: function() {
            ...
            if (!plotData || !plotData.data || plotData.data.length === 0) {
                if (window.AndroidBridge) {
                    window.AndroidBridge.onPlotError('No data provided for violin plot');
                }
                return;
            }
                ...
                this.plotDiv.on('plotly_click', function(data) {
                    if (data.points && data.points.length > 0) {
                        ...
                        if (window.AndroidBridge) {
                            ...
                            window.AndroidBridge.onPointClicked(clickData);
                        }
                    }
                });
                ...
                this.plotDiv.on('plotly_hover', function(data) {
                    if (data.points && data.points.length > 0) {
                        ...
                        if (window.AndroidBridge) {
                            ...
                            window.AndroidBridge.onPointHovered(hoverData);
                        }
                    }
                });
        ...
        setPointPosition: function(jitter) {
            if (!this.plotDiv || !this.currentData.data) return;

            const updatedData = this.currentData.data.map(function(trace) {
                return Object.assign({}, trace, {
                    pointpos: jitter
                });
            });

            Plotly.react(this.plotDiv, updatedData, this.currentLayout, this.currentConfig)
                .then(function() {
                    if (window.AndroidBridge) {
                        window.AndroidBridge.onPlotUpdated('Point position updated');
                    }
                });
        },
        ...
        toggleBox: function(show) {
            ...
            Plotly.react(this.plotDiv, updatedData, this.currentLayout, this.currentConfig)
                .then(function() {
                    if (window.AndroidBridge) {
                        window.AndroidBridge.onPlotUpdated('Box visibility toggled');
                    }
                });
        },
