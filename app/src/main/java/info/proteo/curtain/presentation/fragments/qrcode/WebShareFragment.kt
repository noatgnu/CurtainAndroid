...
import info.proteo.curtain.databinding.FragmentWebShareBinding
...
class WebShareFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(linkId: String, frontendUrl: String): WebShareFragment {
            return WebShareFragment().apply {
    ...
    private fun generateWebUrl() {
        // Remove trailing slash from frontendUrl if present
        val cleanFrontendUrl = frontendUrl.trimEnd('/')
        val webUrl = "$cleanFrontendUrl/#/$linkId"
        
        binding.urlText.text = webUrl
    }
    ...
    private fun shareQRCode() {
        generatedQRImageData?.let { base64Data ->
            try {
                ...
                cachePath.mkdirs()
                val file = File(cachePath, "qr_code_web_$linkId.png")
                ...
                val shareIntent = Intent().apply {
                    ...
                    putExtra(Intent.EXTRA_TEXT, "Access Curtain data: ${binding.urlText.text}")
                    type = "image/png"
    ...
    private fun copyUrlToClipboard() {
        ...
        val clip = android.content.ClipData.newPlainText("Curtain URL", binding.urlText.text)
